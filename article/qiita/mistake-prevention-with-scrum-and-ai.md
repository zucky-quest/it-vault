...
- 結果として、チームの**ベロシティが約75SP → 約150SP（およそ2倍）** に向上した（詳細と注意点は後述）
...
これらの仕組み化を進めた結果、チームの**ベロシティ**（1スプリントで完了できたストーリーポイントの合計）が明確に伸びました。

> **用語補足**
> - **ストーリーポイント（SP）**: タスクの相対的な大きさ（規模・複雑さ）を表す見積もり単位。
> - **ベロシティ**: 1スプリントで完了できたSPの合計。チームの「こなせる量」の目安。

ちょうど **Sprint 72 で、全PRへのAIレビューを必須化**しました。この前後でベロシティが大きく変わっています。

Jira のベロシティボードで見ると、Before / After は次のとおりです。

| 期間 | スプリント | ベロシティ（目安） |
|------|-----------|-------------------|
| **Before** | Sprint 67〜71 | 約 75 SP |
| **After** | Sprint 72 以降 | 約 150 SP |

```text
ベロシティ（SP）

Before  Sprint 67-71  ██████████                 約75
After   Sprint 72-    ████████████████████       約150（およそ2倍）
```

AIレビューと CODEOWNERS で手戻りが減り、レトロの改善が仕組みに積み上がっていったことで、「同じ指摘の再発」や「レビュー待ち・修正の往復」に費やしていた時間が、価値のある開発に回るようになった、と捉えています。

> **注意（数値の読み方）**
> ベロシティはチーム内部の相対指標であり、他チームと比較したり、生産性そのものと同一視したりできる値ではありません。また、この向上は仕組み化だけが要因とは限らず、メンバーの習熟やタスクの性質など複数の要因が重なった結果です。あくまで「仕組み化と同じ時期に、チームのアウトプットが伸びた」という事実として捉えてください。
